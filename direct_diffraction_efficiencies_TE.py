...
def diffraction_efficiency(str_gmsh_path,nm,lambda0,theta,d,eps_sub,eps_sup,npt_integ,nb_slice,N_d_order):
    decalage  = 0
    ...
    Ez_diff_t = np.loadtxt('./Views/sub_field_cuts.out',usecols=[8])+1j*np.loadtxt('./Views/sub_field_cuts.out',usecols=[9])
    Ez_diff_r = np.loadtxt('./Views/sup_field_cuts.out',usecols=[8])+1j*np.loadtxt('./Views/sup_field_cuts.out',usecols=[9])
